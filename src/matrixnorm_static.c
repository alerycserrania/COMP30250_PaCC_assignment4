...
int main(int argc, char *argv[])
{
    ...
    #pragma omp parallel for schedule(static) shared(a, b, c, m_size) private(sum, j, k)
    for (i = 0; i < m_size; i++)
    {
    ...
    norm = 0.0;
    #pragma omp parallel for schedule(static) shared(c, m_size) private(i, sum) reduction(max: norm)
    for (j = 0; j < m_size; j++)
    {
